...
Module.prototype.require = function requireWrapper(packageName) {
  var absolutePath = locator.absolute(packageName, resolveFilename(packageName, this));
  var requiredPackage;
  ...
  try {
    ...
    if (error.code === 'MODULE_NOT_FOUND' && validMassage === error.message) {
      ...
      absolutePath    = resolveFilename(packageName, this);
...
function resolveFilename (packageName, moduleContext) {
  try {
    return Module._resolveFilename(packageName, moduleContext);
  } catch (error) {
    // dependency is not yet registered in npm
    // return the current process path
    // to try to look it up in the work directory
    return process.cwd();
  }
}
